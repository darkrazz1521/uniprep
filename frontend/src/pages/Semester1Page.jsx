...
export default function Semester1Page({ onNavClick }) {
    const handleSubjectClick = (subject) => {
        onNavClick('question-bank', { subject: subject.name, semester: 'Semester 1' });
    };

    return (
        <section className="page active">
            <div className="text-center mb-12">
                <h1 className="text-4xl font-bold text-slate-800">Semester 1 Subjects</h1>
                <p className="mt-2 text-slate-500">Explore your subjects for Semester 1.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {subjects.map((subject, index) => (
                    <div key={subject.code} onClick={() => handleSubjectClick(subject)} className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg cursor-pointer transition-all duration-300 flex flex-col items-center justify-center text-center animate-in" style={{ animationDelay: `${index * 50}ms` }}>
